...
void Message::processMESSAGE () {
    ...
    while (xml.readNextStartElement ()){
        if (xml.name() == "FROM") {
            From = xml.readElementText ().toStdString ();
        } else {
            if (xml.name() == "CONTENT") {
                foreach (const QXmlStreamAttribute &attr, xml.attributes()) {
                    if (attr.name().toString () == QLatin1String("TYPE"))  {
                        Type = attr.value().toString().toStdString();
                    }
                }
                Content = base64_decode(xml.readElementText ().toStdString ());
            } else {
